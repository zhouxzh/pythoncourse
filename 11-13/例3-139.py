...
root = tkinter.Tk()
root.title('在tkinter界面中显示Matplotlib绘制的图形')
root.geometry('630x700+100+100')
root.resizable(False, False)       # 两个方向都不允许修改大小

(tkinter.Label(root, anchor='w', text='随机游走的布朗运动')
 .place(x=10, y=20, width=300, height=20))
# 第一个动画，使用Label组件显示动画
lbFig1 = tkinter.Label(root)
lbFig1.place(x=10, y=40, width=300, height=300)
# c表示散点的数量
r, c = 2, 80
# 散点初始位置，第一行为横坐标，第二行为纵坐标
positions = np.random.randint(-10, 10, (r, c))
# 每个散点的颜色，随机彩色
colors = np.random.random((c,3))
fig1, ax1 = plt.subplots()
scatters = ax1.scatter(positions[0], positions[1],
                       marker='*', s=60, c=colors)
canvas1 = FigureCanvasTkAgg(fig1, master=lbFig1)
canvas1.get_tk_widget().pack(fill=tkinter.BOTH,
                             expand=tkinter.YES)
ax1.set_xlim(-40, 40)
ax1.set_ylim(-40, 40)
def update(i):
    ...
    scatters.set_offsets(positions.T)     # 更新散点位置
    return scatters,

# 创建动画，100毫秒刷新一次
ani_scatters = FuncAnimation(fig1, update,
                             interval=100, blit=True)

# 第二个动画
(tkinter.Label(root, anchor='w', text='闪烁的小星星')
 .place(x=320, y=20, width=300, height=20))
...
canvas2.get_tk_widget().pack(fill=tkinter.BOTH,
                             expand=tkinter.YES)
def update2(i):
    # 对散点符号的颜色、大小、边线颜色进行调整和变化
    scatters2.set_facecolor(np.random.random((N,3)))
    scatters2.set_sizes(np.random.randint(50,200,N))
    scatters2.set_edgecolors(np.random.random((N,3)))
    return scatters2,
ani_scatters2 = FuncAnimation(fig2, update2,
                              interval=300, blit=True)

# 第三个动画
(tkinter.Label(root, anchor='w', text='随机折线图')
 .place(x=10, y=360, width=300, height=20))
...
x3, y3 = [], []
line, = ax3.plot(x3, y3)
canvas3 = FigureCanvasTkAgg(fig3, master=lbFig3)
canvas3.get_tk_widget().pack(fill=tkinter.BOTH,
                             expand=tkinter.YES)
def init3():
    x3.clear()
    y3.clear()
...
def update3(i):
    ...
    line.set_data(x3, y3)
    return line,
ani_line = FuncAnimation(fig=fig3, func=update3,
                         frames=range(0,100,5),
                         init_func=init3, interval=500,
                         blit=True)

# 第四个动画
(tkinter.Label(root, anchor='w', text='动态柱状图')
 .place(x=320, y=360, width=300, height=20))
...
canvas4.get_tk_widget().pack(fill=tkinter.BOTH,
                             expand=tkinter.YES)
def init4():
    bars = ax4.barh(range(1,13), data['2000年'])
    return bars
def update4(year):
    ax4.cla()
    values = data[year]
    ...
    for xx, yy in zip(range(1,13),y):
        ...
        texts.append(ax4.text(3, xx-0.2, f'{x[xx-1]}月',
                              fontproperties='simhei'))
    return list(bars) + [title] + texts
ani_bars = FuncAnimation(fig=fig4, init_func=init4,
                         func=update4, frames=data.keys(),
